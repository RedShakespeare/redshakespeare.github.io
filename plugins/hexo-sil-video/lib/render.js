...
function createRenderer({ playerStart, playerEnd }) {
  ...
  function renderVideoPlayer(video) {
    ...
    const { icon, playIcon, volumeIcon, repeatIcon, feedbackIcon, brightnessIcon, feedbackPlayIcon, feedbackPauseIcon } = createVideoIcons();
...
<aside class="sil-video-player" data-sil-video-player data-sil-video-model="${model}" tabindex="0" aria-label="视频播放器" style="--sil-video-aspect-ratio:${escapeHtml(video.aspectRatio)}">
  <div class="sil-video-player__stage" data-sil-video-stage tabindex="-1">
    ...
    <div class="sil-video-player__toolbar" data-sil-video-controls role="group" aria-label="视频控制" hidden>
      <button class="sil-video-player__button" data-sil-video-action="${PLAYER_ACTIONS.play}" type="button" aria-label="播放" aria-pressed="false">${playIcon}</button>
      <div class="sil-video-player__volume-control">
        <button class="sil-video-player__button" data-sil-video-action="${PLAYER_ACTIONS.mute}" type="button" aria-label="静音" aria-pressed="false">${volumeIcon}</button>
      ...
      <button class="sil-video-player__button" data-sil-video-action="${PLAYER_ACTIONS.repeat}" type="button" aria-label="播放一次" aria-pressed="false">${repeatIcon}</button>
      <div class="sil-video-player__subtitle-control">
        <button class="sil-video-player__button" data-sil-video-action="${PLAYER_ACTIONS.subtitles}" type="button" aria-label="字幕"${subtitleButtonState}>${icon('subtitles', '<path d="M3 5h18v14H3zm2 2v10h14V7zm1 6h5v2H6zm7 0h5v2h-5zM6 9h8v2H6z"/>')}</button>
      ...
      <button class="sil-video-player__button" data-sil-video-action="${PLAYER_ACTIONS.fullscreen}" type="button" aria-label="进入全屏">${icon('fullscreen', '<path d="M4 4h6v2H6v4H4zm10 0h6v6h-2V6h-4zM4 14h2v4h4v2H4zm14 0h2v6h-6v-2h4z"/>')}</button>
    </div>
  </div>
