...
会隐藏视频工具栏中的下载按钮。此开关只控制播放器入口是否显示，不是媒体访问权限或防下载机制；启用时链接
仍按现有行为在新窗口打开视频资源，由浏览器决定播放或保存。

播放中因媒体缓冲暂时卡住时，画面中央会显示“正在加载...”及当前下载速度。清单管理或 legacy 本地视频会以
精确文件大小和浏览器原生缓冲区增量估算流式传输速度，并对短暂的分段更新做两秒平滑；未知大小的外部视频仅在
浏览器提供可测量字节数时显示速度，否则显示 `--KB/s`。恢复播放后 HUD 自动隐藏。
